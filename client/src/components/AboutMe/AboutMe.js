import "./AboutMe.css";

export default function AboutMe() {
    return (<div id="AboutMe" className='panel column'>
        <h2>About the Author</h2>

        <div className="row">
            <div className='column'>

                <div className='column' id='me-block-mobile'>
                    <img id="me" src="https://avatars.githubusercontent.com/u/37508609?v=4" alt="the creator of this website"/>
                </div>

                <div id="mySocials">
                <blockquote id="myline">Hi, I'm Hamish Starling, a Computing student at Imperial College London.</blockquote>
                    <div className="row" id="socials">
                        
                        <div className='spacer'></div>
                        <div className='spacer'></div>
                        <Social link="https://github.com/starswap" imgLink="https://upload.wikimedia.org/wikipedia/commons/c/cd/GitHub-Mark-64px.png" socialName="Github"/>
                        <div className='spacer'></div>
                        <Social link="https://www.linkedin.com/in/hamish-starling-147859235/" imgLink="https://brand.linkedin.com/content/dam/me/brand/en-us/brand-home/logos/In-Blue-Logo.png.original.png" socialName="Linkedin"/>
                        <div className='spacer'></div>
                        <Social link="https://devpost.com/starswap" imgLink="data:image/svg+xml;base64," socialName="Devpost"/>
                        <div className='spacer'></div>
                        <div className='spacer'></div>

                    </div>
                </div>
            </div>

            <div className='column' id='me-block'>
                <img id="me" src="https://avatars.githubusercontent.com/u/37508609?v=4" alt="the creator of this website"/>
            </div>



        </div>
    </div>);
};

function Social(props) {
    return (
        <a href={props.link} target="__blank" className='surroundingButton'>
            <img src={props.imgLink} alt={props.socialName}/>
        </a>
    )
}